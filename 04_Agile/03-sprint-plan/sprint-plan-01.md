<!-- Template เต็มไฟล์สำหรับสร้าง docs/agile/sprint-plan-[NN].md ของ Sprint ไหนก็ได้ -->

<!-- ดึง Story ของ Sprint นี้มาจาก docs/agile/02-sprint-backlog.md -->

<!-- Sprint 1: เปลี่ยนชื่อ sprint-01.md จาก Lab 07 เป็น sprint-plan-01.md แล้วแทนที่เนื้อหาด้วย template นี้ -->

<!-- Sprint 2-4 ในแลปถัดไป: คัดลอกไฟล์นี้ทั้งไฟล์ไปสร้าง sprint-plan-02.md, sprint-plan-03.md, sprint-plan-04.md ตามลำดับ -->

# Sprint [1] Plan

**Sprint Goal:** [เป้าหมายหลักของ Sprint นี้ในหนึ่งประโยค]
**ระยะเวลา:** [2026-09-1] — [2026-09-14]
**Team:** [SQUARE FORGE]

---

## Sprint Backlog

| # | User Story                                                                                                                          | รับผิดชอบ       | MoSCoW    | Estimate (SP) | Status         |
| - | ----------------------------------------------------------------------------------------------------------------------------------- | ------------------------ | --------- | ------------- | -------------- |
| 1 | ในฐานะผู้เล่น ฉันต้องการเข็นรถเข็น เพื่อขนย้ายตัวอย่างไปทดลอง            | Johnny,Looktaw,Kaow,Plub | Must Have | 5             | 🔄 In Progress |
| 2 | ในฐานะผู้เล่น ฉันต้องการเดิน และเดินเร็วได้ เพื่อเล่นเกม                       | Johnny,Looktaw,Kaow      | Must Have | 5             | 🔄 In Progress |
| 3 | ในฐานะผู้เล่น ฉันต้องการพูดคุยกับ NPC และ Narrator เพื่อดำเนินเนื้อเรื่อง | Johnny,Looktaw,Kaow      | Must Have | 8             | 🔄 In Progress |

## Status Legend

- 🔲 Todo
- 🔄 In Progress
- ✅ Done
- ❌ Blocked

---

## Tasks

### Story 1 — [ในฐานะผู้เล่น ฉันต้องการเข็นรถเข็น เพื่อขนย้ายตัวอย่างไปทดลอง]

- [ ] [Task : Spritesheet Cart]  [Owner : L]  [Estimate : 3]  [status: 🔲]
- [ ] [Task : Spritesheet Monster1]  [owner: P]  [Estimate: 2]  [status: 🔄]
- [ ] [Task : Spritesheet Monster2]  [owner: P]  [Estimate: 2]  [status: 🔲]
- [ ] [Task : Spritesheet Tileset of map]  [owner: K]  [Estimate: 5]  [status: 🔄]
- [ ] [Task : Spritesheet Tileset of props]  [owner: K]  [Estimate: 5]  [status: 🔄]
- [ ] [Task : Cart Logic]  [owner: J]  [Estimate: 5]  [status: 🔲]

### Story 2 — [ในฐานะผู้เล่น ฉันต้องการเดิน และเดินเร็วได้ เพื่อเล่นเกม]

- [ ] [Task : Spritesheet Player]  [owner: L]  [Estimate: 4]  [status: 🔄]
- [ ] [Task : Movement Logic]  [owner: J]  [Estimate: 3]  [status: 🔲]
- [ ] [Task : Collision]  [owner: J]  [Estimate: 5]  [status: 🔲]
- [ ] [Task : UI]  [owner: K]  [Estimate: 3]  [status: 🔲]

### Story 3 — [ในฐานะผู้เล่น ฉันต้องการพูดคุยกับ NPC และ Narrator เพื่อดำเนินเนื้อเรื่อง]

- [ ] [Task : Spritesheet NPC]  [owner: L]  [Estimate: 5]  [status: 🔲]
- [ ] [Task : Dialogue Logic]  [owner: J]  [Estimate: 5]  [status: 🔲]
- [ ] [Task : Story timeline]  [owner: J]  [Estimate: 6]  [status: 🔄]
- [ ] [Task : Interact UI]  [owner: K]  [Estimate: 3]  [status: 🔲]

---

## Daily Notes

### [วันที่]

**เมื่อวาน:** ...
**วันนี้:** ...
**Blocked:** ...

---

## Links

- [[docs/gdd/00-concept|GDD Concept]]
- [[docs/agile/01-product-backlog|Product Backlog]]
- [[docs/agile/02-sprint-backlog|Sprint Backlog]]
